...
/* 
  *  Accounts Model 
  *  This file is included in the accounts controller (accounts/index.php).
  *  It has functions that get data from the 'clients' table.
  */
...
function getLevel($clientLevel)
{
  $db = phpmotorsConnect();
  $sql = 'SELECT clientId, clientFirstname, clientLastname, clientEmail, clientLevel, clientPassword FROM clients WHERE clientEmail = :clientEmail';
  $stmt = $db->prepare($sql);
  $stmt->bindValue(':clientLevel', $clientLevel, PDO::PARAM_STR);
  $stmt->execute();
  $clientData = $stmt->fetch(PDO::FETCH_ASSOC);
  $stmt->closeCursor();
  return $clientData;}
